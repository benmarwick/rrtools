...
- this uses `devtools::create()` to create a basic R package with the name `pkgname` (you should use a different one), and then, if you're using RStudio, opens the projec. If you're not using RStudio, it sets the working directory to the `pkgname` directory.
...
- we found that this function can be a little unreliable in RStudio, sometimes giving and erros and not fully enabling git in RStudio, so, to work around this:
...
- the Rmd contains:   
    ...
    + a code of conduct for contributors, including basic instructions for first-timers to git and GitHub    
